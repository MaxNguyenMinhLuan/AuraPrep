...
exports.health = exports.dailyReset = exports.eveningNudge = exports.afternoonNudge = exports.morningNudge = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const emailService_1 = require("./services/emailService");
const userService_1 = require("./services/userService");
const timezoneUtils_1 = require("./utils/timezoneUtils");
const deepLinkGenerator_1 = require("./utils/deepLinkGenerator");
const generateGuardianCopy_1 = require("../../../shared/generateGuardianCopy");
 ...
 */
exports.morningNudge = functions
    .pubsub
    ...
    try {
        ...
        const currentUTCHour = (0, timezoneUtils_1.getCurrentUTCHour)();
        const targetTimezones = (0, timezoneUtils_1.getTimezonesForLocalHour)(8, currentUTCHour);
        if (targetTimezones.length === 0) {
            console.log('No timezones currently at 8 AM UTC');
            return;
        }
        console.log(`Timezones at 8 AM: ${targetTimezones.join(', ')}`);
        const users = await userService_1.userService.getUsersForNudge(targetTimezones, 'morning');
        ...
        for (const user of users) {
            try {
                ...
                const copy = (0, generateGuardianCopy_1.generateGuardianCopy)({
                    guardianType: user.activeCreature.type,
                    guardianName: user.activeCreature.name,
                    nudgeLevel: 'morning',
                    userName: user.email.split('@')[0],
                    currentStreak: user.currentStreak,
                    deepLink: (0, deepLinkGenerator_1.generateDeepLink)({ userId: user._id || '', email: user.email }, JWT_SECRET, APP_URL)
                });
                // Send email
                ...
                await userService_1.userService.updateNudgeSent(user._id || '', 'morning');
                successCount++;
            }
            catch (error) {
 ...
 */
exports.afternoonNudge = functions
    .pubsub
    .schedule('0 * * * *') // Every hour at :00
    .timeZone('UTC')
    .onRun(async (context) => {
    try {
        ...
        const currentUTCHour = (0, timezoneUtils_1.getCurrentUTCHour)();
        const targetTimezones = (0, timezoneUtils_1.getTimezonesForLocalHour)(14, currentUTCHour);
        if (targetTimezones.length === 0) {
            console.log('No timezones currently at 2 PM UTC');
            return;
        }
        console.log(`Timezones at 2 PM: ${targetTimezones.join(', ')}`);
        const users = await userService_1.userService.getUsersForNudge(targetTimezones, 'afternoon');
        ...
        for (const user of users) {
            try {
                ...
                // Update nudge tracking
                await userService_1.userService.updateNudgeSent(user._id || '', 'afternoon');
                successCount++;
            }
            catch (error) {
 ...
 */
exports.eveningNudge = functions
    .pubsub
    .schedule('0 * * * *') // Every hour at :00
    .timeZone('UTC')
    .onRun(async (context) => {
    try {
        ...
        const currentUTCHour = (0, timezoneUtils_1.getCurrentUTCHour)();
        const targetTimezones = (0, timezoneUtils_1.getTimezonesForLocalHour)(20, currentUTCHour);
        if (targetTimezones.length === 0) {
            console.log('No timezones currently at 8 PM UTC');
            return;
        }
        console.log(`Timezones at 8 PM: ${targetTimezones.join(', ')}`);
        const users = await userService_1.userService.getUsersForNudge(targetTimezones, 'evening');
        ...
        for (const user of users) {
            try {
                ...
                // Update nudge tracking
                await userService_1.userService.updateNudgeSent(user._id || '', 'evening');
                successCount++;
            }
            catch (error) {
