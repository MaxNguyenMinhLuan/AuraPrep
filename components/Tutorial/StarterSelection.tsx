    ...
    return (
        <div className={`fixed inset-0 z-[50] flex items-center justify-center p-3 md:p-4 ${confirmed ? 'animate-fadeOut' : 'animate-fadeIn'} pointer-events-none`}>
            <div className="bg-surface border-4 border-highlight rounded-xl shadow-2xl max-w-4xl w-full p-4 md:p-8 animate-reveal pointer-events-auto overflow-y-auto max-h-[95vh] scrollbar-thin">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-6 mb-5 md:mb-8">
                    {starters.map(starter => {
                        ...
                        return (
                            <button
                                ...
                                className={`p-4 md:p-6 rounded-xl border-4 transition-all duration-200 ${
                                    isSelected
                                        ? 'border-highlight bg-highlight/10 scale-102 md:scale-105 shadow-xl'
                            ...
                            >
                                <div className="flex justify-center mb-2 md:mb-4">
                                    <div className={`transform transition-transform ${isSelected ? 'scale-110' : ''}`}>
                                        <PixelCreature creature={starter} evolutionStage={1} pixelSize={isMobile ? 6 : 8} />
                                    </div>
                                </div>

                                <h3 className="text-lg md:text-xl font-bold text-center mb-1 md:mb-2">{starter.names[0]}</h3>
                                <div className="flex justify-center gap-1.5 mb-2 md:mb-3">
                                    <span className={`text-[10px] md:text-xs px-2 py-0.5 md:py-1 rounded ${
                                        starter.rarity === 'Rare' ? 'bg-rare/20 text-rare border border-rare/50' : 'bg-common/20 text-common border border-common/50'
                                    }`}>
                                        {starter.rarity}
                                    </span>
                                    <span className={`text-[10px] md:text-xs px-2 py-0.5 md:py-1 rounded border ${getTypeColor(starter.type)}`}>
                                        {starter.type}
                                    </span>
                                </div>
                                <p className="text-[11px] md:text-xs text-text-dim text-center leading-relaxed">{starter.description}</p>

                                {isSelected && (
                                    <div className="mt-2 md:mt-4 text-center text-highlight text-xs md:text-sm font-bold animate-pulse">
                                        ✓ Selected
                                    </div>
                                )}
                            </button>
                ...
                <div className="flex justify-center">
                    <button
                        ...
                        className={`px-6 py-3 md:px-8 md:py-4 rounded-lg font-bold text-base md:text-lg transition-all ${
                            selectedId === null
                                ? 'bg-text-dark text-text-dim cursor-not-allowed'
                                : 'bg-highlight text-white hover:bg-highlight/90 border-b-4 border-yellow-800 active:border-b-2 active:translate-y-0.5 shadow-lg'
                        }`}
                    >
