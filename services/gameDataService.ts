 ...
 */
export async function respondToFriendRequest(friendId: string, accept: boolean, authToken: string): Promise<{ success: boolean; message?: string; error?: string }> {
  try {
    const res = await fetch(`${API_URL}/game-data/friends/respond`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authToken}`
...
export async function fetchFriendRequests(authToken: string): Promise<any[] | null> {
  try {
    const res = await fetch(`${API_URL}/game-data/friends/requests`, {
      headers: {
        'Authorization': `Bearer ${authToken}`
      }
    });
